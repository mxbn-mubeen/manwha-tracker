...
async function handleUpdate(update: any) {
  ...
  if (text.startsWith("/create")) {
    await handleCreateCommand(chatId, text);
    return;
  }
  if (text.startsWith("/latest")) {
    await handleLatestCommand(chatId, text);
    return;
  }
  if (text.startsWith("/read")) {
    await handleReadCommand(chatId, text);
    return;
  }
...
export async function poll() {
  ...
  if (savedChatId) {
    ...
    console.log(
      "[bot] No alert chat_id in DB yet — send /start to the bot to register.",
    );
  }
  ...
  for (;;) {
    ...
    try {
      ...
      if (
        ...
        console.warn(
          "[bot] getUpdates conflict: another bot instance is still active. Retrying in 10s...",
        );
        await sleep(10000);
        continue;
      }
      console.error("[bot] getUpdates error:", message);
      await sleep(5000);
      continue;
    }
